...
### v6.5 - Session Stability & Multi-Tab Synchronization (June 2026)
- **Multi-Tab Token Security:** Deactivated JWT token rotation (`ROTATE_REFRESH_TOKENS = False`) and token blacklisting after rotation (`BLACKLIST_AFTER_ROTATION = False`) in `settings.py` to prevent concurrent browser tabs from invalidating active refresh tokens and causing blank pages on tab duplication.
- **SPA Routing Integration:** Added a catch-all URL routing rule in `core/urls.py` that routes non-API, non-static URLs directly back to the React Single Page Application (`index.html`), preventing blank pages or 404 errors on manual browser refreshes.
- **Database Connection Pool Preservation:** Prevented database pool exhaustion and connection leaks by adding `django.db.close_old_connections()` at the start of all background/daemon thread functions (preloader, summarizer, title generator, and database loader).
- **WebSocket Lifecycle & Task Cancellation:** Configured Django Channels to propagate chat deletions via a signal to the WebSocket group, allowing `consumers.py` to intercept the delete event, cleanly cancel the background `asyncio.Task` inside `ACTIVE_GENERATIONS`, and close the connection. This prevents ghost writes to the database.
- **Client-Side Message ID Integration:** Re-engineered message identity allocation by generating unique UUIDs on the client-side (`InputBar.jsx` via `crypto.randomUUID()`) for both user messages and optimistic AI generation slots. These client-side IDs are preserved through WebSockets and stored in the database, allowing `ChatRoom.jsx` to perfectly deduplicate active messages and eliminate duplicates.
- **Zustand Infinite Render Loop Fix:** Fixed a React maximum update depth exceeded error in `ChatRoom.jsx` by establishing a static `EMPTY_ARRAY = []` reference for the Zustand state selector fallback.
- **State Switch Caching:** Refactored state handling in `useChatStore.js` and `ChatRoom.jsx` to cache messages, generation status, and connection handlers on maps indexed by `chatId`. Bound WebSocket cleanups strictly to matching chat IDs, resolving new chat dropped message issues and cross-chat message bleeding.
