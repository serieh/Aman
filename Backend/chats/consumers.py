...
async def background_run_agent(user_id, chat_id, message, model_preference, channel_layer, ai_msg_id, user_msg_id=None, persona_id=None):
    with log_context(chat_id=chat_id, user_id=user_id):
        ...
        try:
            if persona_id:
                ...
                await database_sync_to_async(Chat.objects.filter(chat_id=chat_id).update)(persona_id=persona_id, modify_date=timezone.now())

            async for payload in run_agent(str(user_id), chat_id, message, model_preference, "normal", ai_msg_id=ai_msg_id, user_msg_id=user_msg_id, persona_id=persona_id):
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        with log_context(chat_id=self.chat_id, user_id=user_id):
            ...
            task = asyncio.create_task(background_run_agent(self.user.id, self.chat_id, message, model_preference, self.channel_layer, ai_msg_id, user_msg_id, persona_id))
