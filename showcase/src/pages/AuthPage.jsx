...
export default function AuthPage() {
  ...
  const [autoTypeStarted, setAutoTypeStarted] = useState(false);
  ...
  useEffect(() => {
    if (isAutoPlay && isLogin) {
      // Small delay before auto-typing starts so the transition feels natural
  ...
  // Auto-typing script triggered on click
  const startAutoType = async () => {
    if (!isLogin || typingRef.current) return;
    typingRef.current = true;
    setAutoTypeStarted(true);

    const emailStr = "johnhalo@microslop.com";
    const passStr = "12345678";
    
    // Clear fields before typing to ensure a clean slate
    ...
    for (let i = 0; i < emailStr.length; i++) {
      setFormData(prev => ({ ...prev, email: prev.email + emailStr[i] }));
      await new Promise(r => setTimeout(r, 60)); // Fast typing speed
    }
    ...
    for (let i = 0; i < passStr.length; i++) {
      setFormData(prev => ({ ...prev, password: prev.password + passStr[i] }));
  ...
  const handleSubmit = async (e) => {
    ...
    // Simulate slight delay then mock login
    setTimeout(() => {
      login('fake_access', 'fake_refresh', null);
      navigate(isAutoPlay ? '/app?autoPlay=true' : '/app');
      setLoading(false);
    }, 500);
  };
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-5">
            <div className="space-y-4">
              ...
              <div>
                ...
                <input type="email" name="email" autoComplete="off" onClick={startAutoType} required value={formData.email} onChange={handleChange} className={`w-full px-5 py-3.5 rounded-full border ${errors.email ? 'border-red-300 focus:ring-red-500' : 'border-slate-200 dark:border-slate-700 focus:ring-aman-primary'} bg-slate-50 dark:bg-slate-800 focus:bg-white dark:focus:bg-slate-900 focus:ring-2 focus:border-transparent outline-none transition-all dark:text-white font-medium`} placeholder="you@example.com" />
                {renderError('email')}
              </div>
